...
/**
 * Servlet implementation class EditCommitBL_customer
 */
@WebServlet("/EditCommitBL_customer")
public class EditCommitBL_customer extends HttpServlet {
		...
		//Edit_customerで入力したデータをDBで更新
		 try {
			 Class.forName("com.mysql.cj.jdbc.Driver");
				...
				"UPDATE mst_customer SET CUST_KIND=?,CUST_CORP_NAME=?,CUST_CORP_NAME_KANA=?,CUST_DEPARTMENT_NAME=?,CUST_L_NAME=?,CUST_F_NAME=?,CUST_L_NAME_KANA=?,CUST_F_NAME_KANA=?,CUST_ZIP=?,CUST_PREFECTURE=?,CUST_ADDRESS1=?,CUST_ADDRESS2=?,CUST_TEL1=?,CUST_TEL2=?,CUST_TEL3=?,CUST_MOBILE1=?,CUST_MOBILE2=?,CUST_MOBILE3=?,CUST_MAIL=?,CUST_MAIL_SEND_FLG=?,UPDATE_DATE=now(),UPDATE_USER_ID=? where ID=?; ");
			 ...
			 request.getRequestDispatcher("/EditCnfirm_customer.jsp").forward(request, response); 
